...
First `.new` and thus `#initialize`, is called with those same `initialization_args`, or no args if none
...
which may be used via `new` construction as well as `from_xml` construction.
